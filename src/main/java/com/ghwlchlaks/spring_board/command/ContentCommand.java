...
public class ContentCommand implements Command {
	...
	public void execute(Model model) {
		...
		String bId = request.getParameter("bId"); //넘어온 데이터의 bId값 추출
		
		Dao dao = new Dao(); //데이터 베이스 접근 객체 생성
		Dto dto = dao.contentView(bId); //데이터베이스 접근 객체에 id값 전달하여 맞는 데이터 값 가져오기
		
		model.addAttribute("content_view", dto); // 데이터 객체를 content_view에 전달 (response)
	}
